		...
		private void SendXCharge() {
			...
			foreach(int selectedIndex in listSelectedIndices) { 
				if(table.Rows[selectedIndex]["XChargeToken"].ToString()!="" &&
					CreditCards.GetTokenCount(table.Rows[selectedIndex]["XChargeToken"].ToString(),CreditCardSource.XServer)!=1) {
					_failed++;
				...
				long creditCardNum=PIn.Long(table.Rows[selectedIndex]["CreditCardNum"].ToString());

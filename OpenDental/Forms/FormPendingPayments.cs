...
namespace OpenDental {
	public partial class FormPendingPayments:ODForm {
		...
		private void FillGrid() {
			...
			listPaymentsClinic=listPaymentsClinic.OrderBy(x => Clinics.GetDesc(x.ClinicNum))
				.ThenBy(x => x.PayDate)
			...
			foreach(Payment pay in listPaymentsClinic) {
				row=new ODGridRow();
				if(PrefC.HasClinicsEnabled) {
					string clinicDesc=Clinics.GetDesc(pay.ClinicNum);
					row.Cells.Add(clinicDesc=="" ? Lan.g(this,"Unassigned") : clinicDesc );
				}

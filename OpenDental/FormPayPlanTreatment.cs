...
namespace OpenDental{
	...
	public class FormPayPlanTreatment : ODForm {
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			//This means that any changes made to this object here or in FormATP actually change the object in ListPayPlanCredits.
			...
			if(FormATC.DialogResult==DialogResult.OK) {
				//for(int i = 0;i < ListPayPlanCredits.Count();i++) {
				//		ListPayPlanCredits[i].ChargeDate=selectedCharge.ChargeDate;
				//		ListPayPlanCredits[i].Principal=selectedCharge.Principal;
				//		ListPayPlanCredits[i].Note=selectedCharge.Note;
				//		break;
				//	}
				//}
				...
				if(!_listPayPlanProcs.Any(x => x.ProcNum == selectedCharge.ProcNum)) {
				...
				if(!ListPayPlanCredits.Any(x => x.ProcNum == selectedCharge.ProcNum)){
